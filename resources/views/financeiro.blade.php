    ...
    <div class="container bg-white p-6 rounded-xl shadow-lg mx-auto my-8">
        ...
        <div class="flex flex-col lg:flex-row lg:space-x-4 space-y-4 lg:space-y-0 mb-8">
            ...
            <div class="flex-1 bg-slate-300 p-4 rounded-xl shadow-md border border-gray-200 flex flex-col items-center">
                <h3 class="text-xl font-bold text-gray-800 mb-4 text-center">CUSTO REAL DE MATÉRIA PRIMA</h3>
                <input type="text"
                       class="form-input block w-full rounded-md border border-gray-300 shadow-sm bg-white p-3 text-base text-gray-700 mb-4 text-center"
                       placeholder="Custo Real"
                       value="R$ {{ number_format($custoRealMp ?? 0, 2, ',', '.') }}" readonly> 
            </div>
        </div>
                        ...
                        @foreach ($materiasPrima as $materia)
                            <tr>
                                ...
                                <td class="px-3 py-4 whitespace-nowrap text-sm font-medium flex items-center space-x-2">
                                    ...
                                    <a href="{{ route('financeiro.materiaprima.edit', $materia->id) }}" class="text-blue-500 hover:text-blue-700 transition duration-300 ease-in-out">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </a>
                                    
                                    <form action="{{ route('financeiro.materiaprima.destroy', $materia->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta matéria-prima?');" class="inline-block">
                                        @csrf         
                                        @method('DELETE') 
                                        <button type="submit" class="text-red-500 hover:text-red-700 transition duration-300 ease-in-out bg-transparent border-none p-0 cursor-pointer">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
